...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme943
 */

// Auto-rebuild the theme registry during theme development.
if ( theme_get_setting( 'theme943_clear_registry' ) ) {
	// Rebuild .info data.
 ...
 */
function theme943_html_head_alter( &$head_elements ) {
 ...
 */
function theme943_preprocess_search_block_form( &$vars ) {
 ...
 */
function theme943_preprocess( &$vars, $hook ) {
	$vars['theme943_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme943_preprocess_html( &$vars ) {
	$vars['doctype'] = _theme943_doctype();
	$vars['rdf'] = _theme943_rdf( $vars );
	...
	if ( !$vars['is_front'] ) {
		...
		if ( isset( $page_name ) ) {
			$vars['classes_array'][] = theme943_id_safe( 'page-' . $page_name );
		}

		$vars['classes_array'][] = theme943_id_safe( 'section-' . $section );
	...
	if ( ( $d['destination'] == 'blog' ) && ( theme_get_setting( 'theme943_blog_title' ) != "" ) ) {
		$vars['title'] = theme_get_setting( 'theme943_blog_title' );
		$vars['head_title'] = theme_get_setting( 'theme943_blog_title' );
	}
}
 ...
 */
function theme943_preprocess_page( &$vars ) {
 ...
 */
function theme943_preprocess_maintenance_page( &$vars ) {
	...
	drupal_add_css( drupal_get_path( 'theme', 'theme943' ) . '/css/maintenance-page.css' );
}
 ...
 */
function theme943_preprocess_node( &$vars ) {
	// Striping class
 ...
 */
function theme943_preprocess_block( &$vars, $hook ) {
 ...
 */
function theme943_menu_tree( $vars ) {
 ...
 */
function theme943_field__taxonomy_term_reference( $vars ) {
	$output = '';
 ...
 */
function theme943_breadcrumb( $vars ) {
	$breadcrumb = isset( $vars['breadcrumb'] ) ? $vars['breadcrumb'] : array();

	$condition = theme_get_setting( 'theme943_breadcrumb_hideonlyfront' ) ? count( $breadcrumb ) > 1
			: !empty( $breadcrumb );
	$separator = theme_get_setting( 'theme943_breadcrumb_separator' );

	if ( theme_get_setting( 'theme943_breadcrumb_showtitle' ) ) {
 ...
 * @return bool
 */
function theme943_tabs_float() {
	$float = (bool) theme_get_setting( 'theme943_tabs_float' );
	$float_node = (bool) theme_get_setting( 'theme943_tabs_node' );
...
function theme943_id_safe( $string ) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme943_menu_link( array $vars ) {
	...
	$element['#attributes']['class'][] = theme943_id_safe( $element['#title'] );
 ...
 */
function theme943_preprocess_menu_local_task( &$vars ) {
 ...
 */
function theme943_menu_local_tasks( &$vars ) {
	$output = '';
 ...
 */
function theme943_preprocess_comment( &$variables ) {
...
/* Region options */
function theme943_region_preffix ( $reg ) {
	$block_bg_type = theme_get_setting( 'theme943_' . $reg . '_bg_type' );
	$block_bg_img = theme_get_setting( 'theme943_' . $reg . '_bg_img' );
	$block_bg_parallax = theme_get_setting( 'theme943_' . $reg . '_bg_parallax' );
	$bg_video = theme_get_setting( 'theme943_' . $reg . '_bg_video' );
	$bg_video_start = theme_get_setting( 'theme943_' . $reg . '_bg_video_start' );
	$fullwidth = theme_get_setting( 'theme943_' . $reg . '_fullwidth' );
...
function theme943_region_suffix ( $reg ) {
	$fullwidth = theme_get_setting( 'theme943_' . $reg . '_fullwidth' );
...
function theme943_form_contact_site_form_alter( &$form, &$form_state, $form_id ) {
	...
	//$form['subject']['#element_validate'] = array( '_theme943_empty_field' );
	//$form['subject']['#suffix'] = '<div class="messages error">This field is required</div>';
...
/*function _theme943_empty_field ( $element, &$form_state ) {
